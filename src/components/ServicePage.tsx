...
import { ServiceKineticBand } from "@/components/ServiceKineticBand";
  ...
  return (
    <>
      <section className="relative flex min-h-[88vh] items-end overflow-hidden">
        ...
        <MeteorStreaks />
        <div className="pointer-events-none absolute -right-32 top-1/4 h-96 w-96 rounded-full blur-3xl" style={{ background: `color-mix(in srgb, ${service.accent} 14%, transparent)` }} />
        ...
        <div className="relative mx-auto w-full max-w-7xl px-5 pt-32 pb-16 lg:px-8 lg:pb-24">
          <p className="eyebrow">{site.location.city} ({site.location.state}) · {site.location.alsoServes} · Remoto</p>
          ...
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-muted-foreground lg:text-base">{service.summary}</p>
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <WhatsappCta message={service.whatsappMessage} serviceKey={service.key} ctaLocation="hero_servico">Falar sobre {service.label.toLowerCase()}</WhatsappCta>
            <Link to="/servicos" className="inline-flex items-center gap-2 rounded-full border border-border px-5 py-2.5 text-sm text-muted-foreground transition-colors hover:border-tech hover:text-foreground">
      ...
      <ServiceKineticBand service={service} />
      ...
      <section className="relative overflow-hidden border-t border-border bg-surface/30 py-20 lg:py-28">
        <div className="pointer-events-none absolute -left-32 top-16 h-80 w-80 rounded-full blur-3xl" style={{ background: `color-mix(in srgb, ${service.accent} 9%, transparent)` }} />
        <div className="relative mx-auto max-w-7xl px-5 lg:px-8">
          <Reveal>
            <p className="eyebrow">O que entra nesse atendimento</p>
            <h2 className="mt-3 max-w-2xl font-display text-4xl leading-tight text-foreground lg:text-5xl">Escopo claro, combinado antes de começar.</h2>
          </Reveal>

          <ul className="mt-12 grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 250px), 1fr))" }}>
      ...
      <section className="relative overflow-hidden py-20 lg:py-28">
        ...
        <div className="relative mx-auto max-w-7xl px-5 lg:px-8">
          <Reveal>
            ...
            <h2 className="mt-3 max-w-2xl font-display text-4xl leading-tight text-foreground lg:text-5xl">Do primeiro contato até o problema resolvido.</h2>
          </Reveal>
          <ol className="service-process-rail mt-12 grid gap-6 md:grid-cols-3" style={{ "--process-accent": service.accent } as CSSProperties}>
            {process.map((p, i) => (
              <li key={p.title} className="service-process-node">
                <Reveal delay={i * 0.08} className="h-full">
                  ...
                  >
                    ...
                    <div className="mt-5 h-px w-10 origin-left transition-transform duration-500 group-hover:scale-x-150" style={{ background: service.accent }} />
                ...
                <Reveal key={f.q} delay={i * 0.05}>
                  <details className="group py-5">
                    <summary className="cursor-pointer list-none text-base text-foreground marker:hidden">
                      <span className="flex items-start justify-between gap-4">{f.q}<span className="mt-1 font-mono text-xs text-tech transition-transform group-open:rotate-45">+</span></span>
                    </summary>
      ...
      <section className="relative overflow-hidden border-t border-border">
        ...
        <div className="relative mx-auto max-w-3xl px-5 py-24 text-center lg:px-8 lg:py-32">
          <Reveal>
            <h2 className="font-display text-4xl leading-tight text-foreground lg:text-5xl">Precisa de {service.label.toLowerCase()}?</h2>
            <p className="mx-auto mt-4 max-w-xl text-sm text-muted-foreground lg:text-base">Me explique a situação pelo WhatsApp. Se der para resolver remotamente, resolvo remotamente; se precisar de atendimento presencial, combinamos data e local.</p>
            <div className="mt-8 flex justify-center"><WhatsappCta message={service.whatsappMessage} serviceKey={service.key} ctaLocation="cta_final_servico" /></div>
          </Reveal>
        </div>
      </section>
    </>
