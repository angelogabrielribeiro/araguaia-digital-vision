  ...
  return (
    <section ref={ref} className="relative h-[520svh] border-t border-border bg-[#020609]">
      <div className="sticky top-0 h-svh overflow-hidden">
          ...
          <div className="grid w-full items-center gap-10 lg:grid-cols-[0.9fr_1.1fr]">
            <div className="relative z-20 max-w-xl">
              ...
              <div className="mt-5 flex items-center gap-3 font-mono text-[9px] uppercase tracking-[.18em] text-white/38">
                <span>0{active + 1}</span><span className="h-px w-12 bg-white/15" /><span>0{services.length}</span>
              </div>

              <motion.div key={current.key} initial={{ opacity: 0, y: 28, filter: "blur(8px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.52, ease: [0.22, 1, 0.36, 1] }}>
                <h2 className="mt-5 font-display text-[clamp(2.7rem,6vw,5.3rem)] leading-[.92] text-foreground">{current.title}</h2>
              ...
              <div className="mt-10 flex gap-2">
                {services.map((service, index) => (
                  <span key={service.key} className="h-1 rounded-full transition-all duration-500" style={{ width: index === active ? 48 : 16, background: index === active ? service.accent : "rgba(255,255,255,.12)" }} />
                ))}
              </div>
            </div>
            ...
            <div className="relative mx-auto aspect-square w-full max-w-[620px] perspective-[1200px]">
              ...
              <div className="absolute left-1/2 top-1/2 h-28 w-28 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/10" style={{ background: `radial-gradient(circle, ${current.accent}55 0%, ${current.accent}15 38%, transparent 72%)`, boxShadow: `0 0 80px ${current.accent}35` }} />
              ...
              {services.map((service, index) => {
                ...
                return (
                  <motion.div
                    key={service.key}
                    animate={{ left: `${left}%`, top: `${top}%`, scale: isActive ? 1.12 : 0.82, opacity: isActive ? 1 : 0.42, z: isActive ? 80 : -40 }}
        ...
        <div className="pointer-events-none absolute bottom-5 left-1/2 -translate-x-1/2 rounded-full border border-white/10 bg-black/35 px-4 py-2 font-mono text-[8px] uppercase tracking-[.16em] text-white/45 backdrop-blur-md">continue rolando · o mapa responde ao scroll</div>
      </div>
    </section>
  );
}
